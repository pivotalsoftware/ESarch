...
if [ -z ${1+x} ] && [ -z ${2+x} ] && [ -z ${3+x} ] && [ -z ${4+x} ] && [ -z ${5+x} ]; then
    echo "Usage: USERNAME PASSWORD ORG SPACE TRADER-APP-URL"
    exit 1
fi
...
if [ -z ${5+x} ]; then
    echo "I need your Trader App URL to continue."
    exit 1
else
    echo "Trader App URL: $5"
fi
...
cf set-env esrefarch-demo-trader-ui REACT_APP_API_ROOT $5
popd

echo $'UI deployed. You can check its URL below...'
